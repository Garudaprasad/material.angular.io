...
const DOCS: { [key: string]: DocItem[] | DocCategory[] } = {
  [CONVERSIONS]: [
    {
      id: "autocomplete",
      name: "Autocomplete",
...
export class DocumentationItems {
  public hasCategory(section: string): boolean {
    return this.instanceOfDocCategory(DOCS[section][0]);
  }

  instanceOfDocCategory(object: any): object is DocCategory {
    return "id" in object && "name" in object && "items" in object;
  }

  getCategories(section: string): DocCategory[] {
    return DOCS[section] as DocCategory[];
  }

  getItems(section: string): DocItem[] {
    return DOCS[section] as DocItem[];
  }
}
