...
const DOCS: { [key: string]: DocItem[] } = {
  [COMPONENTS]: [
    ...
    },
    {
      ...
      additionalApiDocs: [
        { name: "Testing", path: "material-button-toggle-testing.html" },
      ],
    },
    {
      id: "card",
    ...
    },
    {
      ...
      additionalApiDocs: [
        { name: "Testing", path: "material-progress-spinner-testing.html" },
      ],
    },
    {
      id: "radio",
...
const DOCS_OLD: { [key: string]: DocCategory[] } = {
  [COMPONENTS]: [
    {
      id: "forms",
      name: "Form Controls",
...
for (const category of DOCS_OLD[COMPONENTS]) {
...
for (const category of DOCS_OLD[CDK]) {
  for (const doc of category.items) {
    doc.packageName = "cdk";
    doc.examples = exampleNames.filter(
      (key) =>
...
const ALL_COMPONENTS_OLD = DOCS_OLD[COMPONENTS].reduce(
  (result: DocItem[], category: DocCategory) => result.concat(category.items),
  []
);
const ALL_CDK_OLD = DOCS_OLD[CDK].reduce(
  (result: DocItem[], cdk: DocCategory) => result.concat(cdk.items),
  []
);
const ALL_DOCS_OLD = ALL_COMPONENTS_OLD.concat(ALL_CDK);
const ALL_CATEGORIES_OLD = DOCS_OLD[COMPONENTS].concat(DOCS_OLD[CDK]);

@Injectable()
export class DocumentationItems {
  getCategories(section: string): DocCategory[] {
    return DOCS_OLD[section];
  }
  ...
  getItems_old(section: string): DocItem[] {
    if (section === COMPONENTS) {
      return ALL_COMPONENTS_OLD;
    }
    if (section === CDK) {
      return ALL_CDK_OLD;
    }
